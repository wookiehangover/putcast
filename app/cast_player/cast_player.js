 ...
 */
CastPlayer.prototype.selectMedia = function(model) {
  console.log("media selected", model.toJSON());
  ...
  this.selectMediaUpdateUI(model);
};
 ...
 */
CastPlayer.prototype.loadMedia = function(model) {
  if (!this.session) {
    console.log("no session");
    return;
  }
  console.log("loading..." + model.get('name'));
  var mediaInfo = new chrome.cast.media.MediaInfo(model.streamingUrl());
  ...
  document.getElementById("media_title").innerHTML = model.get('name');
...
CastPlayer.prototype.playMediaLocally = function(currentTime) {
  ...
  if( this.localPlayerState != PLAYER_STATE.PLAYING && this.localPlayerState != PLAYER_STATE.PAUSED ) { 
    this.localPlayer.src = this.model.streamingUrl();
 ...
 */
CastPlayer.prototype.selectMediaUpdateUI = function(model) {
  document.getElementById('video_image').src = model.get('screenshot');
